...
export function CheckinPanel({ t, controller }: PanelProps) {
  ...
  return <section ref={panelRef} className="dsh-checkin ci-panel" id="dsh-checkin-panel" role="dialog" aria-modal="false" aria-labelledby="ci-title">
    ...
    <div className="ci-scroll">
      <div className="ci-toolbar"><select className="ci-select" aria-label={t('filter')} value={topicId} onChange={event => setTopicId(event.target.value as TopicId | '')}><option value="">{t('all')}</option>{data?.topics.map(topic => <option key={topic.id} value={topic.id}>{topic.name}</option>)}</select><button className="ci-button ci-primary" disabled={busy} onClick={() => beginForm()}><Plus size={16} />{t('add')}</button></div>
      ...
      {data && data.topics.length === 0 && <div className="ci-empty"><CalendarCheck2 size={36} strokeWidth={1.4} /><h3>{t('empty')}</h3><p>{t('emptyHint')}</p><button className="ci-button" disabled={busy} onClick={() => beginForm()}><Plus size={16} />{t('add')}</button></div>}
