...
import { recordsTableSql, topicColumns, validateTable } from './database.ts'
...
export class CheckinConnection {
  ...
  private store: CheckinStore | undefined
  private storeSignature: string | undefined
  constructor(private readonly ctx: Context, private readonly config: StoreConfig) {}
  close(): void { this.lifetime.abort(); this.store?.close() }
  private serial<Result>(signal: AbortSignal, action: (signal: AbortSignal) => Promise<Result>): Promise<Result> {
    const combined = AbortSignal.any([signal, this.lifetime.signal])
    const work = this.queue.then(() => { combined.throwIfAborted(); return action(combined) })
    this.queue = work.catch(() => undefined)
    return work
  }
  private external(): StoreConfig | undefined {
    ...
    if (this.config.apiKey === undefined && this.config.databaseId === undefined && !apiKey && !databaseId) return undefined
  ...
  private async saved(): Promise<Saved | undefined> {
    ...
    } catch { throw failure('checkin/credentials-unavailable') }
  }
  private async save(saved: Saved | undefined, expectedRevision: string | undefined): Promise<void> {
    try {
      if (!saved) {
        ...
        await this.provider().modifyRecord(recordKey, async current => {
          ...
          if (previous?.revision !== expectedRevision) throw failure('checkin/connection-changed')
  ...
  private editable(): void { if (this.external()) throw failure('checkin/managed-connection') }
  private async overview(apiKey: string, signal: AbortSignal): Promise<DatabaseResources> {
  ...
  private async state(): Promise<ConnectionState> {
    const external = this.external()
    if (external) {
      let phase: ConnectionState['phase'] = 'connected'
      try { const store = new CheckinStore(external); store.close() } catch { phase = 'invalid' }
    ...
    if (!saved) return { phase: 'login', source: 'none', revision: 'none', writable: true }
  ...
  connection(signal: AbortSignal): Promise<ConnectionState> { return this.serial(signal, () => this.state()) }
  ...
  private async activate(saved: Saved, target: ConnectRequest, signal: AbortSignal): Promise<ConnectionState> {
    ...
    try { await store.query({ from: '0001-01-01', to: '9999-12-31' }, signal) } finally { store.close() }
  ...
  initialize(request: InitializeRequest, signal: AbortSignal): Promise<ConnectionState> {
    return this.serial(signal, async currentSignal => {
      ...
      const validTable = (name: string, existing?: boolean) => (existing ? z.string().regex(/^[a-z][a-z0-9_]{0,63}$/u) : nameSchema).safeParse(name).success
      if (!validTable(request.tableName, request.useExistingTopics) || !validTable(request.recordsTableName, request.useExistingRecords) || request.tableName === request.recordsTableName ||
        ...
        (request.databaseId ? !z.uuid().safeParse(request.databaseId).success : !nameSchema.safeParse(request.databaseName).success)) throw failure('checkin/invalid-config')
      ...
      try {
        ...
        if (!database || database.status === 'provisioning') {
          ...
          const pending: Saved = { ...saved, version: 2, phase: 'setup', pendingDatabaseName: request.databaseName!, tableName: request.tableName, recordsTableName: request.recordsTableName, revision: randomUUID() }
        ...
        const pending: Saved = { ...saved, version: 2, phase: 'setup', databaseId: database.id, tableName: request.tableName, recordsTableName: request.recordsTableName, revision: randomUUID() }
        await this.save(pending, saved.revision)
        saved = pending
        const targets = [
          { name: request.tableName, role: 'topics' as const, existing: request.useExistingTopics },
          { name: request.recordsTableName, role: 'records' as const, existing: request.useExistingRecords },
        ]
        ...
        for (const target of targets.filter(target => !target.existing)) {
          ...
          if (target.role === 'topics') await client.request(`/databases/${encodeURIComponent(database.id)}/tables`, currentSignal, { name: target.name, columns: topicColumns }, true)
          else await client.request(`/databases/${encodeURIComponent(database.id)}/query`, currentSignal, { sql: recordsTableSql(target.name), params: [] }, true)
          await validateTable(client, database.id, target.name, target.role, currentSignal)
        }
