...
import { CalendarCheck2, Check, ChevronLeft, ChevronRight, Pencil, Plus, Trash2, X } from 'lucide-react';
 ...
 */
export function CheckinTrigger({ wide, t, controller }) {
    const state = useSyncExternalStore(controller.subscribe, controller.getSnapshot);
    return _jsxs(_Fragment, { children: [_jsx("style", { children: styles }), _jsxs("button", { className: "ci-trigger", "data-wide": wide, "aria-label": t('open'), "aria-expanded": state.open, "aria-controls": "dsh-checkin-panel", onClick: () => state.open ? controller.close() : controller.open(), children: [_jsx(CalendarCheck2, { size: 16, "aria-hidden": "true" }), wide && _jsx("span", { children: t('title') })] })] });
}
 ...
 */
export function CheckinPanel({ t, controller }) {
    const state = useSyncExternalStore(controller.subscribe, controller.getSnapshot);
    ...
    useEffect(() => {
        ...
        return () => { document.removeEventListener('keydown', key); if (returnFocus.current?.isConnected)
    ...
    useEffect(() => {
        if (!state.open || !data)
            return;
        const poll = () => { if (document.visibilityState === 'visible')
