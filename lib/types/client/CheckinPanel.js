...
export function CheckinPanel({ t, controller }) {
    ...
    useEffect(() => {
        ...
        const outside = (event) => {
            ...
            if (target instanceof Element && target.closest('.ci-trigger[aria-controls="dsh-checkin-panel"]'))
                return;
            restoreFocus = false;
            controller.close();
        };
        ...
        return () => {
            ...
            if (restoreFocus && returnFocus.current?.isConnected)
