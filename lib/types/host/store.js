...
import { assertBackupSize, parseCheckinBackup } from "../backup.js";
...
function parseBackup(json, today) {
    try {
        ...
        if (error instanceof Error && error.message === 'checkin/backup-too-large')
...
export class CheckinStore {
            ...
            if (version === 1)
                this.transaction(() => {
                    ...
                    for (const topic of topics)
                        insertTopic.run(sqliteText(topic.id), sqliteText(topic.name), sqliteText(topic.createdAt), sqliteText(topic.updatedAt), randomUUID());
                    const migratedAt = this.now().toISOString();
                    for (const completion of completions)
                        insertRecord.run(sqliteText(completion.topicId), sqliteText(completion.date), migratedAt, randomUUID());
    ...
    list() {
        return { today: todayInBeijing(this.now()), timeZone: 'Asia/Shanghai', topics: this.db.prepare('SELECT id,name,created_at AS createdAt,updated_at AS updatedAt FROM checkin_topics ORDER BY created_at,id').all() };
    }
    /** @param rawName - User-entered name. @returns Created topic. */
    create(rawName) {
        const name = validName(rawName);
        return this.transaction(() => {
            if (this.db.prepare('SELECT id FROM checkin_topics WHERE name=?').get(name))
    ...
    update(id, rawName) {
        ...
        return this.transaction(() => {
            this.topic(id);
            if (this.db.prepare('SELECT id FROM checkin_topics WHERE name=? AND id<>?').get(name, id))
                throw failure('checkin/duplicate-name');
    ...
    set(request) {
        ...
        return this.transaction(() => {
            ...
            if (request.completed)
                this.db.prepare('INSERT OR IGNORE INTO checkin_records(topic_id,date,created_at,revision) VALUES(?,?,?,?)').run(topic.id, date, this.now().toISOString(), randomUUID());
            else
                this.db.prepare('DELETE FROM checkin_records WHERE topic_id=? AND date=?').run(topic.id, date);
    ...
    query(request) {
        ...
        return this.transaction(() => {
            ...
            const completions = (request.topicId === undefined
                ...
                : this.db.prepare('SELECT topic_id AS topicId,date FROM checkin_records WHERE date BETWEEN ? AND ? AND topic_id=? ORDER BY date').all(from, to, request.topicId));
    ...
    importData(json) {
        ...
        return this.transaction(() => {
            ...
            if (incoming.some(topic => findName.get(topic.name) !== undefined))
                throw failure('checkin/import-conflict');
            const insertTopic = this.db.prepare('INSERT INTO checkin_topics(id,name,created_at,updated_at,revision) VALUES(?,?,?,?,?)');
            const insertRecord = this.db.prepare('INSERT INTO checkin_records(topic_id,date,created_at,revision) VALUES(?,?,?,?)');
            for (const topic of incoming)
                insertTopic.run(topic.id, topic.name, topic.createdAt, topic.updatedAt, randomUUID());
            ...
            for (const item of completions)
                insertRecord.run(item.topicId, item.date, item.createdAt, randomUUID());
            return {
